    ...
    <div class="px-5 py-4 lg:px-8">
        <div class="flex items-center justify-between gap-4">
            ...
            <div class="flex items-center gap-4 flex-1 justify-end max-w-md">
                ...
                <form action="{{ route('search') }}" method="GET" class="relative w-full hidden md:block">

                    {{-- Ubah SVG jadi Button Submit biar bisa diklik --}}
                    <button type="submit"
                        class="absolute left-3 top-1/2 -translate-y-1/2 text-gray-400 hover:text-black transition-colors">
                    ...
                    {{-- Tambah name="q" dan value="{{ request('q') }}" --}}
                ...
                <a href="{{ route('cart.index') }}"
                    class="relative cursor-pointer hover:scale-105 transition-transform group">
                    <div
                        class="w-10 h-10 rounded-full bg-black text-white flex items-center justify-center group-hover:bg-gray-800 transition-colors">
                        <svg class="w-5 h-5" fill="none" stroke="currentColor" viewBox="0 0 24 24">
                            <path stroke-linecap="round" stroke-linejoin="round" stroke-width="2"
                    ...
                    @if ($cartCount > 0)
                        <span
                            class="absolute -top-1 -right-1 w-5 h-5 bg-red-500 rounded-full text-white text-xs flex items-center justify-center border-2 border-white font-bold">
                            {{ $cartCount }}
                        </span>
                    @endif
                </a>
                ...
                <div class="hidden lg:relative lg:block" x-data="{ open: false }">
                    <button @click="open = ! open" class="flex items-center focus:outline-none">
                        <div
                            class="w-10 h-10 rounded-full bg-gray-200 overflow-hidden cursor-pointer hover:ring-2 hover:ring-gray-300 transition-all">
        ...
        <form action="{{ route('search') }}" method="GET" class="mt-4 relative md:hidden">
            ...
            {{-- Tambah name="q" dan value="{{ request('q') }}" --}}
            <input type="text" name="q" value="{{ request('q') }}" placeholder="Search styles..."
